...
if command fup 2>/dev/null; then
    ...
    && dtr && sudo zypper up --details \
    && etr && sudo zypper up --details \
    && sudo zypper dup --details --from Packman\ Repository --allow-vendor-change \
    && sudo rpmconf -a \
    && sudo btrfs fi usage / && lff \
...
function llp {
if command llp 2>/dev/null; then
    command llp
else
...
function cua {
if command cua 2>/dev/null; then
    command cua
else
    conda update --all
...
function lff {
if command lff 2>/dev/null; then
    command lff
else
...
function llf {
if command llf 2>/dev/null; then
    command llf
else
...
# List all gpg keys installed from package repos
function lgk {
if command lgk 2>/dev/null; then
    command lgk
else
    rpm -q gpg-pubkey --qf \
...
function dsc {
if command dsc 2>/dev/null; then
    command dsc
else
...
function etr {
if command etr 2>/dev/null; then
    command etr
else
    sudo zypper mr -e libdvdcss\ repository && i=1 \
    && sudo zypper mr -e home:megamaced:spotify-easyrpm && i=$((i+1)) \
...
function dtr {
if command dtr 2>/dev/null; then
    command dtr
else
    ...
    && sudo zypper mr -d editors && i=$((i+1)) \
...
function cnl {
if command cnl 2>/dev/null; then
    command cnl
else
